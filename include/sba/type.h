...
namespace SBA {
   ...
   // 返回指定区域（r）的边界。side 决定是返回边界的最小值还是最大值。
   ...
   /* -------------------------------------------------------- */
   // 表示一个单位的标识符，通常用于表示寄存器、内存区域等的唯一标识符
   class UnitId {
    public:
